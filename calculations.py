...
def graphics(data, today, chat_id):
    ...
    # Калории
    bars_calories = sns.barplot(x=["Потреблено", "Сожжено"], y=[calorie_in, calorie_out], ax=axs[0], hue=["Потреблено", "Сожжено"])
    axs[0].axhline(y=calorie_goal, color="#FF6347", linewidth=2)  # Цель
    axs[0].set(title="Калории",
               ylabel='Количество калорий, ккал')

    # Вода
    bars_water = sns.barplot(x=["Выпито"], y=water, ax=axs[1])
    axs[1].axhline(y=water_goal, color="#FF6347", linewidth=2)  # Цель
    axs[1].set(title="Вода",
               ylabel='Количество воды, мл')
    ...
    axs[0].legend(handles=handles_calories, labels=labels_calories, loc="best",
                  title="Калории")
    ...
    axs[1].legend(handles=handles_water, labels=labels_water, loc="best",
                  title="Вода")
...
def global_calorie(calorie_df):
    ...
    sns.lineplot(data=calorie_df, x="date", y="calorie", hue="calorie_type", palette=new_palette)
...
def global_water(water_df):
    ...
    sns.lineplot(data=water_df, x="date", y="water", hue="water_type", palette=new_palette)
