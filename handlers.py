...
from calculations import calculate_calorie_target, calculate_water_norm, activity_water, graphics, get_mass_index, global_calorie, global_water
...
def add_today_data(users, chat_id, today):
    ...
    if today not in users[chat_id]:
        ...
        if users[chat_id][today]['calorie_goal_type'] == "calc": # Если автоматом считали калории, пересчитываем
            users[chat_id][today]["calorie_goal"] = int(calculate_calorie_target(weight=users[chat_id][today]["weight"],
                                                                             height=users[chat_id][today]["height"],
                                                                             age=users[chat_id][today]["age"],
                                                                             activity=users[chat_id][today]["activity"],
                                                                             ))
        users[chat_id][today]["water_goal"] = int(calculate_water_norm(users[chat_id][today]["weight"], users[chat_id][today]['city'], users[chat_id][today]["activity"]))
        users[chat_id][today]["logged_water"] = 0  # Сбрасываем данные воды
        users[chat_id][today]["logged_calories"] = 0  # Сбрасываем данные калорий
        users[chat_id][today]["burned_calories"] = 0  # Сбрасываем данные сожжённых калорий
...
async def cmd_start(message: Message):
    await message.answer("Вас приветствует фитнес-бот!"
...
async def update_state_and_ask(state, key, value, next_state, message, question):
    ...
    await message.answer(question) # Для этого задаём вопрос, чтобы ответ на него был обновлением
...
async def set_profile_start(message: Message, state: FSMContext):
    ...
    await message.answer("Пожалуйста, введите ваш вес (в кг):") # Вопрос, ответ на который - вес
...
async def set_weight(message: Message, state: FSMContext):
    try:
        ...
        await update_state_and_ask(state, "weight", weight, ProfileSetup.height, message, "Введите ваш рост (в см):")
    except ValueError:
...
async def set_height(message: Message, state: FSMContext):
    try:
        ...
        await update_state_and_ask(state, "height", height, ProfileSetup.age, message, "Введите ваш возраст:")
    except ValueError:
...
async def set_age(message: Message, state: FSMContext):
    try:
        ...
        await update_state_and_ask(state, "age", age, ProfileSetup.activity, message, "Сколько минут активности у вас в день?")
    except ValueError:
...
async def set_activity(message: Message, state: FSMContext):
    try:
        ...
        await update_state_and_ask(state, "activity", activity, ProfileSetup.city, message,
...
async def set_city(message: Message, state: FSMContext):
    city = message.text
    await update_state_and_ask(state, "city", city, ProfileSetup.calorie_goal, message,
...
async def set_calorie_target(message: Message, state: FSMContext):
    ...
    users[chat_id] = {today:{
        ...
        "water_goal": int(calculate_water_norm(data["weight"], data['city'], data["activity"])),
        "logged_water": 0,
        "logged_calories": 0,
        "burned_calories": 0,
...
async def cmd_my_propfile(message: Message):
    ...
    if add_today_data(users, chat_id, today):
        ...
        await message.answer("Вы не создавали профиль. Воспользуйтесь командой /set_profile для его создания.")
...
async def cmd_log_water(message: Message):
    ...
    try:
        ...
        await message.answer(f"Выпито {water} мл воды. Осталось выпить {users[chat_id][today]['water_goal'] - users[chat_id][today]['logged_water']} мл воды.")
    except (IndexError, ValueError):
        await message.answer("Используй команду так: /log_water <количество воды в мл>.")
...
async def cmd_log_food(message: Message, state: FSMContext):
    ...
    if info:
        ...
        full_name = await translate(info["foods"][0]["food_name"], lang="en", lang_to="ru")
        await state.set_state(Food.eat_weight) # Готовимся установить состояние - вес пищи
        await state.update_data(calories_per_100_g=calories_per_100_g)
        await message.answer(f"{full_name} — {int(calories_per_100_g)} ккал на 100 г. Сколько грамм вы съели?")
    else:
        await message.answer(f"Не удалось распознать продукт '{name}'. Попробуйте задать его иначе.")
...
async def eat_weight(message: Message, state: FSMContext):
    ...
    try:
        ...
        await state.update_data(eat_weight=eat_weight) # Устанавливаем состояние - вес пищи
        data_eat = await state.get_data() # Сохраняем состояние в переменную
        eat_calories = int(data_eat['eat_weight']/100 * data_eat['calories_per_100_g']) # Рассчитываем калораж с учётом веса пищи
...
async def cmd_log_workout(message: Message):
    ...
    try:
        ...
        activity_data = get_exercise_data(english_activity_name, users[chat_id][today]['weight'],
                                          users[chat_id][today]['height'],
                                          users[chat_id][today]['age'],
                                          )
        if activity_data:
            activity_calorie = int(activity_data[0]['nf_calories'] * time_activity/activity_data[0]['duration_min'])
            activity_name_api = await translate(activity_data[0]['name'], lang="en", lang_to="ru")
            ...
            await message.answer(f"Не удалось распознать активность '{activity_name}'. Попробуйте задать её иначе.")
    except (IndexError, ValueError):
        await message.answer(f"Пожалуйста, введите команду в формате /log_workout <тип тренировки> <время (мин)>.")
...
async def cmd_recomend(message: Message):
    ...
    if not add_today_data(users, chat_id, today):
        await message.answer("Сначала настройте профиль с помощью /set_profile.")
        return
    imt = round(get_mass_index(users[chat_id][today]['weight'], users[chat_id][today]['height']), 2)
    if imt >= 25:
        eng_names_food = get_low_calorie()
        food_recomendation = "\n".join([f"✔️ {await translate(food, lang="en", lang_to="ru")} (100 грамм ≈ {get_food_data(food)["foods"][0]['nf_calories']} ккал)" for food in eng_names_food])
        await message.answer(f"Ваш ИМТ выше нормы (ИМТ = {imt}).\n"
                             f"🍏 Рекомендуемые низкокалорийные продукты:\n"
                            f" {food_recomendation}")
        return
    elif imt < 25 and imt > 18.5:
        await message.answer(f"Ваш ИМТ в пределах нормы (ИМТ = {imt}).\n"
                             f"🍏 ✔️ Рекомндаций не требуется.\n")
    else:
        await message.answer(f"Ваш ИМТ ниже нормы (ИМТ = {imt}).\n"
...
async def cmd_global(message: Message):
    ...
    chat_id = str(message.chat.id)
    if not add_today_data(users, chat_id, today):
        await message.answer("У вас нет истории. Настройте профиль с помощью /set_profile, чтобы её начать.")
        return
    if len(users[chat_id]) == 1:
        await message.answer("Вы используете бот первый день. "
                             "Используйте бот хотя бы два дня. "
                             "Для отслеживания прогресса за день используйте /check_progress")
        return

    df = pd.DataFrame.from_dict(users[chat_id], orient='index').reset_index() # Создание датафрейса из данных пользователя
    ...
    input_file_calorie = BufferedInputFile(graph_calorie.read(), filename="global_calorie.png")
    graph_calorie.close()
    graph_water = global_water(water_df)
    input_file_water = BufferedInputFile(graph_water.read(), filename="global_water.png")
    graph_water.close()
...
async def cmd_edit_profile(message: types.Message, state: FSMContext):
    ...
    if not add_today_data(users, chat_id, today):
        await message.answer("У вас нет профиля. Настройте профиль с помощью /set_profile")
        return

    # Создание кнопок для параметров
    keyboard = InlineKeyboardBuilder()
    parameters_visible = ["Вес", "Рост", "Возраст", "Активность", "Город", "Цель по калориям"]
    for visible_param in parameters_visible:
        keyboard.add(InlineKeyboardButton(text=visible_param, callback_data=f"change_{visible_param}"))
    keyboard.adjust(2)  # Настройка количества кнопок в ряду
    await message.answer("Выберите параметр для изменения:", reply_markup=keyboard.as_markup())
...
async def process_change(callback: CallbackQuery, state: FSMContext):
    ...
    param = parameters[parameters_visible.index(visible_param)]  # Извлекаем внутреннее имя параметра

    # Сохраняем выбранный параметр в состояние
    await state.update_data(selected_params=[param, visible_param])

    await callback.message.answer(f"Введите новое значение для параметра {visible_param}:")
...
async def set_value(message: Message, state: FSMContext):
    ...
    if selected_params[0] == "weight":
        try:
            ...
            await update_user_parameter(users, chat_id, today, selected_params[0], new_value)
            calorie_goal = int(calculate_calorie_target(
                weight=users[chat_id][today]["weight"],
                height=users[chat_id][today]["height"],
                age=users[chat_id][today]["age"],
                activity=users[chat_id][today]["activity"]))
            await update_user_parameter(users, chat_id, today, 'calorie_goal', calorie_goal)
            water_goal = int(calculate_water_norm(users[chat_id][today]["weight"], users[chat_id][today]['city'],
                                                  users[chat_id][today]["activity"]))
            await update_user_parameter(users, chat_id, today, 'water_goal', water_goal)
            await message.answer(f"Пользовательское изменение:\n\n"
                                 f"⚖️ Параметр {selected_params[1]} успешно обновлен на {new_value} кг.\n\n"
                                 f"Вслед за ним изменились параметры:\n\n"
                                 f"🔥 Цель по калориям на {calorie_goal} ккал. "
                                 f"Вы можете изменить этот параметр вручную.\n\n"
                                 f"💦 Норма воды на {water_goal} мл.")
        except ValueError:
    ...
    if selected_params[0] == "height":
        try:
            ...
            await message.answer(f"Пользовательское изменение:\n\n"
                                 f"📏 Параметр {selected_params[1]} успешно обновлен на {new_value} см.\n\n"
                                 f"Вслед за ним изменился параметр:\n\n"
                                 f"🔥 Цель по калориям на {calorie_goal} ккал. "
                                 f"Вы можете изменить этот параметр вручную.")
        except ValueError:
    ...
    if selected_params[0] == "age":
        try:
            ...
            users[chat_id][today]["calorie_goal_type"] = "calc"
            await message.answer(f"Пользовательское изменение:\n\n"
                                 f"🗓 Параметр {selected_params[1]} успешно обновлен на {new_value} лет.\n\n"
                                 f"Вслед за ним изменился параметр:\n\n"
                                 f"🔥 Цель по калориям на {calorie_goal} ккал. "
                                 f"Вы можете изменить этот параметр вручную.")
        except ValueError:
            await message.answer("Пожалуйста, введите число для изменения возраста в годах.")
            return
    if selected_params[0] == "activity":
        try:
            ...
            await update_user_parameter(users, chat_id, today, 'calorie_goal', calorie_goal)
            users[chat_id][today]["calorie_goal_type"] = "calc"
            water_goal = int(calculate_water_norm(users[chat_id][today]["weight"], users[chat_id][today]['city'], users[chat_id][today]["activity"]))
            await update_user_parameter(users, chat_id, today, 'water_goal', water_goal)
            await message.answer(f"Пользовательское изменение:\n\n"
                                 f"🏃 Параметр {selected_params[1]} успешно обновлен на {new_value} мин.\n\n"
                                 f"Вслед за ним изменились параметры:\n\n"
                                 f"🔥 Цель по калориям на {calorie_goal} ккал. Вы можете изменить этот параметр вручную.\n\n"
                                 f"💦 Норма воды на {water_goal} мл.")
        except ValueError:
            await message.answer(
    ...
    if selected_params[0] == "city":
        try:
            ...
            await update_user_parameter(users, chat_id, today, 'water_goal', water_goal)
            await message.answer(f"Пользовательское изменение:\n\n"
                                 f"🌃 Параметр {selected_params[1]} успешно обновлен на {new_value}.\n\n"
                                 f"Вслед за ним изменился параметр:\n\n"
                                 f"💦 Норма воды на {water_goal} л.")
        except ValueError:
    ...
    if selected_params[0] == "calorie_goal":
        try:
            ...
            users[chat_id][today]["calorie_goal_type"] = "calc"
            await message.answer(f"Пользовательское изменение:\n\n"
                                 f"🔥 Параметр {selected_params[1]} успешно обновлен на {new_value} ккал.")
        except ValueError:
            await message.answer("Пожалуйста, введите число для изменеия цели по калориям в ккал.")
            return
